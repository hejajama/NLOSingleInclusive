...
double I1(const RunParameters& rp, double r, double y){
  double res=(Sr_interp_2D(r,y)*(2*log(c0/r)-log(rp.mu2)))/(2*M_PI);
...
double H1(const RunParameters& rp, double r, double y){
  double res=(Sq(Sr_interp_2D(r,y))*(2*log(c0/r)-log(rp.mu2)))/M_PI;
  if(rp.alpha_s_running==PARENT || rp.alpha_s_running==DAUGHTER || rp.alpha_s_running==SMALLEST){
    return alpha_s_pos(r)*res;
  }else{
    return res;
...
double integrand_xi(double xi, void *userdata){
  ...
  //
  double xi2=Sq(xi);
  double xq_xi=(xi>xp ? xf(rp,xp/xi,rp.mu2) : 0);
  double xq_xi1=xf(rp,xp,rp.mu2);
  double xg_xi=(xi>xp ? xf(rp,xp/xi,rp.mu2) : 0);
  double xg_xi1=xf(rp,xp,rp.mu2);
  //
  double res=0;
  if(rp.with_CF){
    double real_CF=0;
    double virt_CF=-M_1_PI*(2*log(k) - log(rp.mu2) + 2*log(1-xi))*Sr_interp_2D(r,y);
    if(rp.alpha_s_running==PARENT || rp.alpha_s_running==DAUGHTER || rp.alpha_s_running==SMALLEST){
        virt_CF*=alpha_s_pos(r);
        real_CF+=(xi>xp ? I1(rp,r,y)+I1(rp,xi*r,y) - 4*I2_interp(r,y) : 0);
    }
    else if(rp.alpha_s_running==MIXED || rp.alpha_s_running==MIXEDBD){
        virt_CF*=alpha_s_mom(k);
        real_CF+=(xi>xp ? (I1(rp,r,y)+I1(rp,xi*r,y) - 4*I2_interp(r,y))*alpha_s_mom(k) : 0);
    }
    else{
        real_CF+=(xi>xp ? I1(rp,r,y) + I1(rp,xi*r,y) - 4*I2_interp(r,y) : 0);
    }
  ...
  if(rp.with_gl){
    double resgl1 = (xi>xp ? 0.25*H1(rp,r,y) + 0.5*I1(rp,xi*r,y) : 0);
  ...
  if(rp.with_gq){
    double resgq1 = (xi>xp ? 0.25*H1(rp,xi*r,y) + 0.5*I1(rp,r,y) : 0);
  ...
  if(rp.with_gg){
    double gg11 = (xi>xp ? H1(rp,r,y) + H1(rp,xi*r,y) : 0);
    double gg12 = (xi>xp ? - H2_interp(r,y) : 0);
    double gg2 = H3_interp(r,y) - M_1_PI*(2*log(k)-log(rp.mu2)+2*log(1-xi))*Sq(Sr_interp_2D(r,y));
    double gg3 = H5_interp(r,y) - 0.5*M_1_PI*(2*log(k)-log(rp.mu2)+2*log(1-xi))*Sq(Sr_interp_2D(r,y));
...
double integral_xi(const RunParameters& rp, double r, double xp, double xg, double k){
  IntegrandXiContext ctx{&rp, r, xp, xg, k};
  gsl_function F;
...
void init_xi_int_interp(const RunParameters& rp, double xp, double xg, double k){
  using namespace amplitude;
  using namespace interp_xi_int;
  double *intvals=new double[rpoints];
  for(int i=0; i<rpoints; i++){
    double r=rvals[i];
    intvals[i]=integral_xi(rp,r,xp,xg,k);
  }
